...
QInputMethodEvent *
QWaylandInputMethodEventBuilder::buildCommit(const QString &text)
{
	...
	if (m_cursor != 0 || m_anchor != 0) {
		QString surrounding = QInputMethod::queryFocusObject(
					      Qt::ImSurroundingText, QVariant())
					      .toString();
		const int cursor = QInputMethod::queryFocusObject(
					   Qt::ImCursorPosition, QVariant())
					   .toInt();
		const int anchor = QInputMethod::queryFocusObject(
					   Qt::ImAnchorPosition, QVariant())
					   .toInt();
		const int absoluteCursor =
			QInputMethod::queryFocusObject(Qt::ImAbsolutePosition,
						       QVariant())
				.toInt();

		const int absoluteOffset = absoluteCursor - cursor;

		const int cursorAfterCommit =
			qMin(anchor, cursor) + replacement.first + text.size();
		...
		attributes.push_back(QInputMethodEvent::Attribute(
			...
			indexFromWayland(surrounding, m_cursor,
					 cursorAfterCommit) +
				absoluteOffset,
			indexFromWayland(surrounding, m_anchor,
					 cursorAfterCommit) +
				absoluteOffset,
			QVariant()));
	}
...
QInputMethodEvent *
QWaylandInputMethodEventBuilder::buildPreedit(const QString &text)
{
	QList<QInputMethodEvent::Attribute> attributes;
	...
	if (m_preeditCursor < 0) {
		...
		attributes.append(QInputMethodEvent::Attribute(
			...
			indexFromWayland(text, m_preeditCursor), 1,
			QVariant()));
	}

	for (const QInputMethodEvent::Attribute &attr :
	     std::as_const(m_preeditStyles)) {
		int start = indexFromWayland(text, attr.start);
		int length = indexFromWayland(text, attr.start + attr.length) -
			     start;
		attributes.append(QInputMethodEvent::Attribute(
			attr.type, start, length, attr.value));
	}

	QInputMethodEvent *event = new QInputMethodEvent(text, attributes);

	const QPair<int, int> replacement = replacementForDeleteSurrounding();
	event->setCommitString(QString(), replacement.first,
			       replacement.second);
...
QWaylandInputMethodEventBuilder::replacementForDeleteSurrounding()
{
	...
	const QString &surrounding = QInputMethod::queryFocusObject(
					     Qt::ImSurroundingText, QVariant())
					     .toString();
	const int cursor =
		QInputMethod::queryFocusObject(Qt::ImCursorPosition, QVariant())
			.toInt();
	const int anchor =
		QInputMethod::queryFocusObject(Qt::ImAnchorPosition, QVariant())
			.toInt();
	...
	const int deleteAfter =
		indexFromWayland(surrounding, m_deleteAfter, selectionEnd) -
		selectionEnd;
...
QWaylandInputMethodContentType::convertV3(Qt::InputMethodHints hints)
{
						      ...
						      int length, int base)
{
	...
	if (length < 0) {
		const QByteArray &utf8 =
			QStringView{ text }.left(base).toUtf8();
		return QString::fromUtf8(
			       utf8.left(qMax(utf8.size() + length, 0)))
			.size();
	} else {
...
int QWaylandInputMethodEventBuilder::trimmedIndexFromWayland(
	const QString &text, int length, int base)
{
	if (length == 0)
		return base;
	...
	if (length < 0) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (ch < 0x80 || ch > 0xbf)
				return QString::fromUtf8(utf8.left(start + i))
					.size();
		}
	} else {
		const QByteArray &utf8 = QStringView{ text }.mid(base).toUtf8();
		...
		for (int i = 0; i < 4; i++) {
			...
			if (ch < 0x80 || ch > 0xbf)
				return base +
				       QString::fromUtf8(utf8.left(start - i))
					       .size();
		}
	}
	return -1;
